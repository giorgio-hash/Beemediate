...
public final class OaFValidator implements OaFValidatorIF{
	...
	/*@ public normal_behaviour
	  ...
	  @ ensures CUSTOMER_NUMBER!=null;
	  @ ensures QUANTITY_MEASURE == 'M';
	  @ ensures DELIVERY_LOCATION_NUMBER!=null & DELIVERY_LOCATION_NUMBER.length==2;
	...
	/*@ public normal_behaviour
	  ...
	  @ requires ost.header.buyerID.length()!=CUSTOMER_NUMBER.length() | ost.header.buyerIDRef.length()!=CUSTOMER_NUMBER.length();
	  @ ensures !\result;
	  @
	  @ also public normal_behaviour
	  @ requires ost.header!=null;
	  @ requires ost.header.buyerID.length()==ost.header.buyerIDRef.length() & ost.header.buyerID.length()==CUSTOMER_NUMBER.length();
	  @ requires (\exists int i; 0<=i<ost.header.buyerID.length(); ost.header.buyerIDRef.charAt(i) != CUSTOMER_NUMBER.charAt(i) | ost.header.buyerIDRef.charAt(i) != CUSTOMER_NUMBER.charAt(i) );
	  @ ensures  !\result;
	  @
	  @ also public normal_behaviour
	  @ requires ost.header!=null;
	  ...
	  @ requires ost.header.buyerID.length()==ost.header.buyerIDRef.length() & ost.header.buyerID.length()==CUSTOMER_NUMBER.length();
	  @ requires (\forall int i; 0<=i<ost.header.buyerID.length(); ost.header.buyerIDRef.charAt(i) == CUSTOMER_NUMBER.charAt(i));
	  @ requires (\forall int i; 0<=i<=ost.header.buyerIDRef.length(); ost.header.buyerIDRef.charAt(i) == CUSTOMER_NUMBER.charAt(i) );
...
//	@CodeBigintMath
	...
	/*@ public normal_behaviour
	  @ requires ost.header!=null;
	  @ requires DELIVERY_LOCATION_NUMBER!=null & DELIVERY_LOCATION_NUMBER.length==2;
	  @ requires (\forall int i; 0<=i<2; DELIVERY_LOCATION_NUMBER[i]!=null & DELIVERY_LOCATION_NUMBER[i].length()>0);
	  @ requires (\forall int i; 0<=i<2; ost.header.deliveryID.length()!=DELIVERY_LOCATION_NUMBER[i].length())
	  @				| (\forall int i; 0<=i<2; ost.header.deliveryIDRef.length()!=DELIVERY_LOCATION_NUMBER[i].length());
	  @ ensures !\result;
	  @
	  @ also public normal_behaviour
	  @ requires ost.header!=null;
	  ...
	  @ requires DELIVERY_LOCATION_NUMBER!=null & DELIVERY_LOCATION_NUMBER.length==2;
	  @ requires (\forall int i; 0<=i<2; DELIVERY_LOCATION_NUMBER[i]!=null & DELIVERY_LOCATION_NUMBER[i].length()>0);
	  @ requires (\exists int i; 0<=i<2; ost.header.deliveryID.length()==DELIVERY_LOCATION_NUMBER[i].length()
	  @										& (\forall int j; 0<=j<ost.header.deliveryID.length(); ost.header.deliveryID.charAt(j)==DELIVERY_LOCATION_NUMBER[i].charAt(j) ));
	  @	requires ost.header.deliveryID.length() == ost.header.deliveryIDRef.length();
...
//	@CodeBigintMath
	...
	/*@ public normal_behaviour
	  ...
	  @ ensures !\result <==> (\exists int i; 0<=i<ost.itemList.length; ost.itemList[i].orderUnit.length()!=1 | ost.itemList[i].orderUnit.charAt(0)!=QUANTITY_MEASURE  );
	  @*/
	private /*@ spec_public pure @*/ boolean validateQuantityMeasure(/*@ non_null @*/final OrderStructure ost) {
		...
		/*@ loop_writes rightMeasureUnit;
		  @ loop_invariant \count < ost.itemList.length ==> il!=null;
		  @ loop_invariant 0<=\count<=ost.itemList.length;
		  @ loop_invariant rightMeasureUnit <==> (\forall int j; 0<=j<\count; ost.itemList[j].orderUnit.length()==1 & ost.itemList[j].orderUnit.charAt(0) == QUANTITY_MEASURE );
		  @ decreases ost.itemList.length - \count;
		  @*/
		for(OrderItem il : ost.getItemList()) {
